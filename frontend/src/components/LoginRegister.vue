...
const triggerMockGoogleLogin = async (customEmail = null) => {
  // Generate a mock base64url-encoded JWT token
  const header = btoa(JSON.stringify({ alg: "HS256", typ: "JWT" }))
    .replace(/=/g, '').replace(/\+/g, '-').replace(/\//g, '_')
  
  const email = customEmail || (activeTab.value === 'signup' ? 'newplayer.google.mock@playconnect.com' : 'ajith@playconnect.com')
  const name = email.split('@')[0]
  const sub = `google-mock-${name}`
...
const handleGoogleClick = () => {
  if (isLocalDev.value) {
    const email = window.prompt("Enter your registered Google email to log in:", "ajith@playconnect.com")
    if (email) {
      triggerMockGoogleLogin(email.trim())
    }
  } else {
    if (typeof window !== 'undefined' && window.google) {
      window.google.accounts.id.prompt((notification) => {
        if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
          const msg = 'Google Sign-In prompt was blocked or closed.'
          if (activeTab.value === 'signup') {
            signupError.value = msg
          } else {
            loginError.value = msg
      ...
      const msg = 'Google SDK is not loaded. Please try again.'
      if (activeTab.value === 'signup') {
        signupError.value = msg
      } else {
        loginError.value = msg
  ...
  <div class="auth-container scrollable-y animate-fade-in">
    ...
    <div v-if="activeTab === 'signin'" class="form-panel animate-fade-in">
      ...
      <div class="social-buttons" style="justify-content: center;">
        <button type="button" class="social-btn google circular" @click.prevent.stop="handleGoogleClick">
